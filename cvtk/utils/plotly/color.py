...
def _hist2d_dataframe(nparr, a, b, n=256):
    matrix, x1, y1, x2, y2 = _hist2d_matrix(nparr, a, b, n)

    rows = pd.Index(np.arange(y1, y2), name=f"c{a}")
    cols = pd.Index(np.arange(x1, x2), name=f"c{b}")
...
def color_hist(img_path, start=0, limit=1, mode="BGR"):
    ...
    for img_path in img_list[start: start + limit]:
        ...
        for i in (0, 1, 2):
            hist = _hist(nparr[..., i])
            axs[i].plot(hist / (hist.max() + 1))
            axs[i].set_title(f"Color [{mode[i]}] value")
...
def color_hist2d(img_path, start=0, limit=1, mode="BGR"):
    ...
    for a, b in [(0, 1), (0, 2), (1, 2)]:
        df = _hist2d_dataframe(nparr, a, b, 256)
